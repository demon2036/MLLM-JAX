...
- 保持无侵入开发：可复用实现放 `plugins/`，项目特定实现/配置/数据放 `projects/`，不要直接改动 `easydel/` 或其他上游仓库。
...
- **可复用实现放 `plugins/`，项目特定实现放 `projects/`**：接口通常放 `plugins/**/api/` 或 `plugins/common/`；项目入口/配置放 `projects/**`；上游目录只做最小必要 glue（如 import / wiring），避免侵入式改动。
