...
from language.gemma.transformer import build_positions_from_mask
...
class Sampler:
  ...
  def init_cache(self, bsz) -> dict[str, ]:
  ...
  def init_sample_state(
    ...
    positions = build_positions_from_mask(input_mask)
  ...
  def tokenize(self, input_string: str) -> jax.Array:
    ...
    input_ids = jnp.array(
        [257152]*256+[self.vocab.bos_id()] + jnp.array(input_ids).tolist(), dtype=jnp.int32
    )
    return input_ids
  ...
  def _sample_fn(
    ...
    while cond_fn(initial_sampling_state):
        print(initial_sampling_state.cache['layer_0']['end_index'])
        initial_sampling_state=self._sample_step(params,initial_sampling_state)
  ...
  def __call__(
    ...
    all_input_ids = [self.tokenize("<image>" +x) for x in input_strings]
    ...
    sampling_state=self._sample_fn( self.params, initial_sampling_state)
