...
class TrainingConfig:
    ...
    sample_from_buffer_prob: float = 0.2
...
# 修改后的 apply_chat_template 函数（用于处理对话模板）
def apply_chat_template(tokenizer: PreTrainedTokenizerBase, conversation_history: List[Dict[str, str]], completion: bool = False) -> str:
    """Applies the chat template to a given conversation history.
    If completion is True, do not add generation prompt.
    """
...
def run_generation_step(
    ...
    params_for_sample=state.params if not use_ema else state.ema_params
    params_dp_bf16 = params_to_dp(jax.tree_util.tree_map(lambda x: jnp.astype(x, jnp.bfloat16), params_for_sample))
...
def main():
    ...
    for step in range(config.training_steps):
        ...
        if step==20:
            state=jax_setup['state']
            jax_setup['state']=state.replace(  ema_params=copy.deepcopy(state.params)   )
