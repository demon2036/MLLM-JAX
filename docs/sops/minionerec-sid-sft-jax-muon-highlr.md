...
- **Environment (verified)**:
  - Commit (baseline runs): `john/sid-sft-muon-20260130` @ `5f53cca`
  - Commit (LR scan script/configs): `nano-gpt` / `origin/nano-gpt-sft` @ `ba1752e`
...
- (Optional) Run a small Muon `(lr, aux_lr)` scan to find the best Muon point.
...
### 8) Focused Muon LR/aux LR scan (started, then interrupted)

This is a follow-up scan using `scripts/run_sid_sft_muon_sweep_v6e8_lr_scan.sh` (commit `ba1752e`).

- Create a v6e-8 TPU VM (spot):
  - `TPU_NAME="sid-sft-muon-sweep-v6e-8-spot-260130063927"; ./scripts/create_tpu_vm.sh --type v6e-8 --zone us-east5-b --name "$TPU_NAME" --spot` (exit `0`)
...
- Sync repo to the target commit:
...
- Clone upstream MiniOneRec (data paths):
  - `scripts/ssh_tpu_vm_root.sh --name "$TPU_NAME" --zone us-east5-b --command 'set -euo pipefail; cd /root/MLLM-JAX; mkdir -p workdir; if [ ! -d workdir/MiniOneRec/.git ]; then git clone https://github.com/AkaliKong/MiniOneRec workdir/MiniOneRec; fi'` (exit `0`)
- Sync W&B `.env`:
  - `./scripts/sync_env_to_tpu_vm.sh --name "$TPU_NAME" --zone us-east5-b --src .env --dest /root/.env --worker all` (exit `0`)
- Start scan (then interrupted at step 40 due to user request):
  - `scripts/ssh_tpu_vm_root.sh --name "$TPU_NAME" --zone us-east5-b --env-file /root/.env --command 'set -euo pipefail; export PYTHONUNBUFFERED=1; export HF_HUB_ENABLE_HF_TRANSFER=1; rm -f /tmp/libtpu_lockfile || true; source /root/miniconda3/etc/profile.d/conda.sh; conda activate mllm-jax; cd /root/MLLM-JAX; bash scripts/run_sid_sft_muon_sweep_v6e8_lr_scan.sh'` (exit `130`)
  - W&B run (partial): `https://wandb.ai/johntitordemon2036/minionerec-sid-sft/runs/zs4u1o7c`
- Delete TPU VM:
