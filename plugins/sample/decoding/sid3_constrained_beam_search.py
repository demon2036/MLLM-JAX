...
def constrained_beam_search_sid3(
    *,
    ...
    Note: the underlying attention cache write index (`end_index`) is effectively
    batch-shared, so `prompt_true_len` must be a scalar shared across the batch.
    For mixed true prompt lengths within a batch, use
    `constrained_beam_search_sid3_prefill` instead.
    """
...
def constrained_beam_search_sid3_prefill(
    *,
    ...
    """Constrained beam search for 3-token SIDs using sampler-style prefill buckets.

    This variant supports mixed true prompt lengths within a batch by:
    - Right-padding prompts to a fixed prefill length (L_prefill = prompt_input_ids.shape[1]).
    - Prefilling the cache to L_prefill.
    - Appending generated tokens at physical positions starting from L_prefill.
    - Using per-sample position_ids (RoPE) and attention masks to ignore the
      "hole" between true_len..L_prefill-1.
    """
    ...
    m2 = int(scores1.shape[1])
    scores1 = scores1.reshape((bsz, k * m2))
    top1_scores, top1_idx = jax.lax.top_k(scores1, k=k)
    ...
    m3 = int(scores2.shape[1])
    scores2 = scores2.reshape((bsz, k * m3))
    top2_scores, top2_idx = jax.lax.top_k(scores2, k=k)
