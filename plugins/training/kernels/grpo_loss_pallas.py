    ...
    if tail_vocab == 0:
        ...
        def kernel_full_and_tail(
            ...
            def _full():
                for sb in range(num_index_subblocks):
                    if use_bf16_softmax:
                        logits_sub = logits_full_ref[0, :, sb * index_subblock : (sb + 1) * index_subblock].astype(
                            jnp.bfloat16
                        )
                        log_softmax_sub = logits_sub.astype(jnp.float32) - lse_val[:, None]
                        probs_sub = jnp.exp(log_softmax_sub).astype(jnp.bfloat16)
        ...
        in_specs = [
            pl.BlockSpec(
                ...
                (lambda b, t, k: (b, t, jnp.minimum(k, full_blocks - 1))) if full_blocks > 0 else (lambda b, t, k: (b, t, 0)),
            ),
