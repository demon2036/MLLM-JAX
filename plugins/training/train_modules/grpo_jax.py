...
class TrainGRPOModuleJax(nn.Module):
    ...
    def __call__(self, inputs):
        ...
        logits_f32 = logits[..., :-1, :].astype(jnp.float32)
        per_token_logps = jnp.take_along_axis(
            jax.nn.log_softmax(logits_f32, axis=-1),
            chosen_ids[..., None],
            axis=-1,
        )[..., 0]
        per_token_logps = per_token_logps / float(self.temperature)
        ...
        probs = jax.nn.softmax(logits_f32 / float(self.temperature), axis=-1)
