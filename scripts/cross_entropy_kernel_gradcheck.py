...
@dataclass(frozen=True)
class CrossEntropyKernelGradcheckConfig:
    config_path: str
    ...
    kernel: CrossEntropyKernelConfig = CrossEntropyKernelConfig(block_size=2048, time_block=8, ignore_index=-100, temperature=1.0)
    tolerances: GradcheckTolerances = GradcheckTolerances()

    wandb_project: str = "mllm-jax-ce-kernel"
    wandb_mode: str = "online"
    wandb_name: str = "ce_kernel_gradcheck_qwen25_1p5b"


def _cfg_from_dict(cfg: dict[str, Any], *, config_path: str) -> CrossEntropyKernelGradcheckConfig:
    ...
    ignore_index = int(cfg.get("ignore_index") or -100)
    ...
    wandb_project = str(cfg.get("wandb_project") or "mllm-jax-ce-kernel")
    wandb_mode = str(cfg.get("wandb_mode") or "online")
    wandb_name = str(cfg.get("wandb_name") or "ce_kernel_gradcheck_qwen25_1p5b")

    return CrossEntropyKernelGradcheckConfig(
        ...
        kernel=CrossEntropyKernelConfig(
...
def _load_config(config_path: str) -> CrossEntropyKernelGradcheckConfig:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if git_sha:
        cfg = CrossEntropyKernelGradcheckConfig(**{**cfg.__dict__, "wandb_name": f"{cfg.wandb_name}_{git_sha}"})
    ...
    print("model:", {"model_path": cfg.model_path, "vocab": vocab})
    print("load_s:", round(t_load, 3))

    key = jax.random.PRNGKey(int(cfg.seed))
    key_ids, key_ign = jax.random.split(key, 2)
    ...
    labels = _to_device0(labels)

    # Randomly mark a small fraction of tokens as ignored.
    ignore_mask = jax.random.bernoulli(key_ign, p=0.1, shape=labels.shape)
    labels = jnp.where(ignore_mask, ignore_index, labels)
    ...
    def _scalar_loss_ref(l):
        per_loss, per_logp = cross_entropy_per_token_reference(
            l,
            labels,
            ignore_index=ignore_index,
            temperature=cfg.temperature,
        )
        loss = (per_loss * mask).sum() / (denom + 1e-8)
        return loss, (per_loss, per_logp)

    def _scalar_loss_kernel(l):
        per_loss, per_logp = cross_entropy_per_token_pallas(
            logits=l,
            labels=labels,
            cfg=cfg.kernel,
            interpret=False,
    ...
    loss_and_grad_ref = jax.jit(jax.value_and_grad(_scalar_loss_ref, has_aux=True))
    loss_and_grad_kernel = jax.jit(jax.value_and_grad(_scalar_loss_kernel, has_aux=True))

    t2 = time.time()
    (loss_ref, (per_loss_ref_fwd, per_logp_ref_fwd)), dlogits_ref = loss_and_grad_ref(logits_for_loss)
    t_ref = time.time() - t2

    t3 = time.time()
    (loss_kernel, (per_loss_kernel_fwd, per_logp_kernel_fwd)), dlogits_kernel = loss_and_grad_kernel(logits_for_loss)
    ...
    metrics = {
        ...
        "kernel/time_block": int(cfg.kernel.time_block),
    ...
    if not isfinite(max_abs) or not isfinite(max_rel) or not isfinite(mean_abs):
        raise SystemExit(f"non-finite dlogits diff: max_abs={max_abs} max_rel={max_rel} mean_abs={mean_abs}")
