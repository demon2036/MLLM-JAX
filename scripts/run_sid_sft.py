...
def _cfg_from_dict(cfg: dict[str, Any], *, config_path: str) -> SidSftConfig:
    ...
    data = SidSftDataConfig(
        ...
        train_pad_policy=str(_get_or_default(cfg, "data.train_pad_policy", DEFAULT_CONFIG["data"].get("train_pad_policy") or "fixed")),
        train_pad_buckets=tuple(int(x) for x in _get_or_default(cfg, "data.train_pad_buckets", DEFAULT_CONFIG["data"].get("train_pad_buckets") or [128, 256, 512])),
        train_pad_to_multiple_of=int(_get_or_default(cfg, "data.train_pad_to_multiple_of", DEFAULT_CONFIG["data"].get("train_pad_to_multiple_of") or 8)),
