...
# Start MiniOneRec RL from a YAML config via `nohup` on a multi-host TPU VM.
#
...
usage() {
  cat <<'USAGE'
Start MiniOneRec RL via nohup on a multi-host TPU VM.

Usage:
  bash scripts/tpu_vm_start_minionerec_rl_from_config_multihost_nohup.sh --config projects/minionerec/rl/configs/<file>.yaml
...
exec bash scripts/tpu_vm_start_minionerec_rl_from_config_nohup.sh \
  --env-name "$ENV_NAME" \
  --config "$CONFIG_PATH" \
  --run-mode "$RUN_MODE"
