...
# Start MiniOneRec RL (GRPO-style) from a YAML config via `nohup` on a TPU VM.
#
...
usage() {
  cat <<'USAGE'
Start MiniOneRec RL via nohup from an explicit YAML config.

Usage:
  bash scripts/tpu_vm_start_minionerec_rl_from_config_nohup.sh --config projects/minionerec/rl/configs/<file>.yaml
...
LOG_FILE="logs/nohup_minionerec_rl_${CONFIG_TAG}_${RUN_MODE}_${HOST_TAG}_${RUN_ID}.log"
EXIT_FILE="logs/nohup_minionerec_rl_${CONFIG_TAG}_${RUN_MODE}_${HOST_TAG}_${RUN_ID}.exit"
PID_FILE="logs/nohup_minionerec_rl_${CONFIG_TAG}_${RUN_MODE}_${HOST_TAG}_${RUN_ID}.pid"

LATEST_LOG="logs/nohup_minionerec_rl_${CONFIG_TAG}_${RUN_MODE}_${HOST_TAG}_latest.log"
LATEST_EXIT="logs/nohup_minionerec_rl_${CONFIG_TAG}_${RUN_MODE}_${HOST_TAG}_latest.exit"
LATEST_PID="logs/nohup_minionerec_rl_${CONFIG_TAG}_${RUN_MODE}_${HOST_TAG}_latest.pid"
...
RUNNER="set -euo pipefail; \
  ...
  cd '$REPO_DIR'; \
  set +e; \
  python -u scripts/run_minionerec_rl.py --config '$CONFIG_PATH' --run-mode '$RUN_MODE'; \
  status=\$?; \
  set -e; \
