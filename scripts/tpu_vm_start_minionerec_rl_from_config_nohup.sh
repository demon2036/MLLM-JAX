...
# checkout (e.g. `/root/MLLM-JAX`). It starts the RL job in the background via
...
Usage:
  bash scripts/tpu_vm_start_minionerec_rl_from_config_nohup.sh \
    --config projects/minionerec_rl/configs/<file>.yaml
...
RUNNER="set -euo pipefail; \
  ...
  cd '$REPO_DIR'; \
  set +e; \
  python -u projects/minionerec_rl/scripts/run_train.py --config '$CONFIG_PATH' --run-mode '$RUN_MODE'; \
  status=\$?; \
  set -e; \
