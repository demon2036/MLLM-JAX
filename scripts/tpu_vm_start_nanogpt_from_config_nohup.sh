...
# Start nanoGPT (JAX/Flax) training from a YAML config via `nohup` on a TPU VM.
#
...
usage() {
  cat <<'USAGE'
Start nanoGPT training via nohup from an explicit YAML config.

Usage:
  bash scripts/tpu_vm_start_nanogpt_from_config_nohup.sh --config projects/nano_gpt/configs/tinyshakespeare_char_v4_8_smoke.yaml
...
LOG_FILE="logs/nohup_nanogpt_${CONFIG_TAG}_${RUN_ID}.log"
EXIT_FILE="logs/nohup_nanogpt_${CONFIG_TAG}_${RUN_ID}.exit"
PID_FILE="logs/nohup_nanogpt_${CONFIG_TAG}_${RUN_ID}.pid"

LATEST_LOG="logs/nohup_nanogpt_${CONFIG_TAG}_latest.log"
LATEST_EXIT="logs/nohup_nanogpt_${CONFIG_TAG}_latest.exit"
LATEST_PID="logs/nohup_nanogpt_${CONFIG_TAG}_latest.pid"
...
RUNNER="set -euo pipefail; \
  ...
  cd '$REPO_DIR'; \
  set +e; \
  python -u projects/nano_gpt/run.py --config '$CONFIG_PATH'; \
  status=\$?; \
  set -e; \
