...
def gen_answers_jax(prompts, sampler, params, max_length_sample,complete=False):
    prompt = []

    if complete:
        for x in prompts:
            prompt.append(x+'\nUser: I think this is not correct.'+'\nAssistant:')
    ...
    outputs = sampler.generate(input_ids_pad, pad_attention, pad_position_ids, prefill_length,
                               max_length=max_length_sample, params=params)
    ...
    for i, (true_length_prompt, step) in enumerate(zip(true_length_prompts, outputs['local_sample_step'])):
        output = \
            sampler.tokenizer.batch_decode(
        ...
        train_input_ids[i, true_length_prompt:true_length_prompt + step + 1] = outputs['local_token_buffer'][i,
        ...
        train_attention_mask[i, true_length_prompt:true_length_prompt + step + 1] = outputs['local_attention_mask'][i,
                                                                                    prefill_length:prefill_length + step + 1]
...
def main():
    replay_buffer = defaultdict(list)
    ...
    state, sampler, train_state_sharding = get_state(mesh_fsdp, training_steps, model_path=model_path,
                                                     grad_accum_steps=grad_accum_steps, num_pre_q=num_pre_Q,
    ...
    for step in range(training_steps):
        ...
        if step>=1:
            for k,v in replay_buffer.items():
                prompts=repeat(k,num_pre_Q)
                break

        tip_text, answers, datas = gen_answers_jax(prompts, sampler,
                                                   params_to_dp(
                                                       jax.tree_util.tree_map(lambda x: jnp.astype(x, jnp.bfloat16),
                                                                              state.params)),
        ...
        metrics = dict()
        mean_correct_length = ema_decay * mean_correct_length + (1 - ema_decay) * completion_ids_global_correct.sum(
            axis=1).max()
        ...
        if step==0:
            for inp, q, a, c in zip(repeated_inputs, tip_text, answers, correct_mask_local):
                if not c:
                    replay_buffer[inp].append([q, a])
